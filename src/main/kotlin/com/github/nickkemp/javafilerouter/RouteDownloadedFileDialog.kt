...
class RouteDownloadedFileDialog(
    ...
    private val identicalFiles   = mutableListOf<ChangedFile>()
    ...
    private fun showDiffPicker() {
        if (changedFiles.isEmpty() && identicalFiles.isEmpty()) return

        val dialog = object : DialogWrapper(project, true) {
            init {
                title = "File Viewer — Changed & Identical"
                init()
            }
            ...
            override fun createCenterPanel(): JComponent {
                ...
                if (changedFiles.isNotEmpty()) {
                    val changedModel = DefaultListModel<String>()
                    changedFiles.forEach { changedModel.addElement(it.fileName) }

                    val changedList = JList(changedModel).apply {
                        selectionMode = ListSelectionModel.SINGLE_SELECTION
                        font = editorFont
                        selectedIndex = 0
                    }

                    val viewDiffBtn = JButton("View Diff").apply {
                        addActionListener {
                            val index = changedList.selectedIndex
                            if (index >= 0) openDiff(changedFiles[index])
                        }
                    }

                    changedList.addMouseListener(object : java.awt.event.MouseAdapter() {
                        override fun mouseClicked(e: java.awt.event.MouseEvent) {
                            if (e.clickCount == 2) {
                                val index = changedList.selectedIndex
                                if (index >= 0) openDiff(changedFiles[index])
                    ...
                    val changedPanel = JPanel(BorderLayout(4, 4))
                    changedPanel.add(JBLabel("Double-click or select and click View Diff:"), BorderLayout.NORTH)
                    changedPanel.add(JBScrollPane(changedList).apply {
                        preferredSize = Dimension(500, 300)
                    }, BorderLayout.CENTER)
                    changedPanel.add(viewDiffBtn, BorderLayout.SOUTH)

                    tabs.addTab("Changed  (${changedFiles.size})", changedPanel)
                }

                // ── Identical tab ─────────────────────────────────────────
                if (identicalFiles.isNotEmpty()) {
                    val identicalModel = DefaultListModel<String>()
                    identicalFiles.forEach { identicalModel.addElement(it.fileName) }

                    val identicalList = JList(identicalModel).apply {
                        selectionMode = ListSelectionModel.SINGLE_SELECTION
                        font = editorFont
                        selectedIndex = 0
                    }

                    val viewIdentBtn = JButton("View Content").apply {
                        addActionListener {
                            val index = identicalList.selectedIndex
                            if (index >= 0) openIdentical(identicalFiles[index])
                        }
                    }

                    identicalList.addMouseListener(object : java.awt.event.MouseAdapter() {
                        override fun mouseClicked(e: java.awt.event.MouseEvent) {
                            if (e.clickCount == 2) {
                                val index = identicalList.selectedIndex
                                if (index >= 0) openIdentical(identicalFiles[index])
                    ...
                    val identicalPanel = JPanel(BorderLayout(4, 4))
                    identicalPanel.add(JBLabel("Files identical to project — double-click to view content:"), BorderLayout.NORTH)
                    identicalPanel.add(JBScrollPane(identicalList).apply {
                        preferredSize = Dimension(500, 300)
                    }, BorderLayout.CENTER)
                    identicalPanel.add(viewIdentBtn, BorderLayout.SOUTH)

                    tabs.addTab("Identical  (${identicalFiles.size})", identicalPanel)
                }
    ...
    private fun openIdentical(identical: ChangedFile) {
        val factory = DiffContentFactory.getInstance()
        val left:  DiffContent = factory.create(identical.existingText)
        val right: DiffContent = factory.create(identical.downloadedText)
        val request = SimpleDiffRequest(
            "${identical.fileName}  —  Files are identical",
            left,
            right,
            "Current (project)",
            "Downloaded (identical)"
        )
        DiffManager.getInstance().showDiff(project, request)
    }
    ...
    private fun analyseFiles() {
        ...
        showDiffsAction.isEnabled = changedCount > 0 || identicalCount > 0
    ...
    private fun collectJava(file: File, sourceRoots: List<File>) {
        ...
        val status = when {
            ...
            target.readText() == content -> {
                identicalFiles.add(ChangedFile(className, content, content))
