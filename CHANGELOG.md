...
- IDENTICAL files now shown in analysis output under their package group alongside CHANGED and NEW files
- Show Diffs now has two tabs: Changed (side-by-side diff) and Identical (side-by-side content view with "Files are identical" title)
- Show Diffs button enabled when either changed or identical files exist, not just changed
- Identical files can be viewed to understand why they matched — e.g. a local change made before the download
